...
@Service
public class WireRopeServiceImplAPI9A2011 implements WireRopeServiceAPI9A2011{
...
//	private TensileStrengthMapper tensileStrengthMapper;
	...
	@Resource
	private WireAttributesApi9a2011Mapper attApiMapper;
	
	
	public Response<WireRope> judgeWireRopeAPI9A2011(WireRope wireRope){
		...
		if(wireRope==null) {
			...
			if("IPS".equals(wireRope.getStrengthLevel())) {
				wireRope.setStrengthLevel("1770");
			}else if("EIP".equals(wireRope.getStrengthLevel())) {
				wireRope.setStrengthLevel("1960");
			}else if("EEIP".equals(wireRope.getStrengthLevel())) {
			 ...
			 * */
			//查询填充系数
			WireAttributesApi9a2011 att = attApiMapper.selectByFilling(wireRope.getStructure());
			boolean flag = true;
...
//						
	...
	private Map<Double, int[]> judgeWireRope(WireRope wireRope){
		...
		if(surface.equals("UB")||surface.equals("B")) {
			diaSurface="U,B";
		}
...
//		}
		...
		StrengthDeviation sd=null;//抗拉强度判定
		...
		while(it.hasNext()) {
			...
			sd=sdMapper.selectByDiamate(wireRopeData);//抗拉强度的上限
...
//				}
			...
			for(WireData wireData:dlist) {
				...
				if(Math.abs(Arith.sub(diamete,ndiamate))>dd.getValue()) {
					re[0]++;//直径不合格数量加1
					all[0]++;
					message = 1;
					wireData.setDiameteJudge("**");
				...
				if("圆形股".equals(shape)) {
					 ...
					 * 
					 */
					if(wireData.getTensileStrength()<wireStrengthLevel-50) {
						re[2]++;//不合格
						all[2]++;
						message = 1;
						wireData.setStrengthJudge("**");
...
//						double k1=strengthLevel*Math.PI*Math.pow((wireData.getnDiamete1()/2.0), 2)*0.45;
					 ...
					 * 
					 */
			    	if(wireData.getTensileStrength()>wireStrengthLevel*0.95) {
						re[2]++;
						all[2]++;
						message = 1;
						wireData.setStrengthJudge("**");
